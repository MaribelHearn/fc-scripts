    ...
    commands: {
        ...
        **/
        ...
        reload: function (src, channel, command) {
            var script = (command[1] ? command[1] + ".js" : "");
            var path;

            if (script && script != "main") {
                if (!sys.fexists("scripts/" + script) && !sys.fexists("plugins/" + script)) {
                    helpers.starfox(src, channel, command, bots.command, "Error 404, that script does not exist.");
                    return;
                } else if (sys.fexists("scripts/" + script)) {
                    path = "scripts/" + script;
                } else if (sys.fexists("plugins/" + script)) {
                    path = "plugins/" + script;
                }
                ...
                try {
                    ...
                    require(path);
                    sys.sendHtmlOwner(helpers.bot(bots.script) + "Script file " + script + " has been reloaded.");
                } catch (e) {
                    print("An error occurred while reloading script file " + script + ": " + e);
                    sys.sendHtmlOwner(helpers.bot(bots.script) + "An error occurred while reloading script file " + script + ": " + e);
                }
        ...
        setgithubkey: function (src, channel, command) {
            ...
            if (!sys.fexists(".git")) {
                helpers.starfox(src, channel, command, bots.script, "Error 403, you cannot set a GitHub API key without a git clone. Please clone <tt>fc-scripts</tt> in your server folder.");
                return;
            }
            var key = command[1];
            if (!key) {
        ...
        update: function (src, channel, command) {
            ...
            if (!sys.fexists(".git")) {
                helpers.starfox(src, channel, command, bots.script, "Error 403, you cannot update the scripts without a git clone. Please clone <tt>fc-scripts</tt> in your server folder.");
                return;
            }
            var auto = false;
            var silent = (command[0].substr(0, 6) == "silent");
            script.updateScript(auto, silent);
        },
        ...
        **/
        serversettings: function (src, channel, command) {
            ...
            + "<b>IP:</b> " + script.hostLocation.ip + "<br>";
            if (API_KEY !== "") {
        ...
        **/
        ...
        registerall: function (src, channel, command) {
            var cusercommands = require("scripts/cusercmds.js").commands;
            var cmodcommands = require("scripts/cmodcmds.js").commands;
            var cownercommands = require("scripts/cownercmds.js").commands;
