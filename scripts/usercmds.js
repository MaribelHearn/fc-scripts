...
/**
    ...
    **/
        commands: function (src, channel, command) {
            ...
            if (require.cache.hasOwnProperty("plugins/funcmds.js") || require.cache.hasOwnProperty("plugins/party.js") || require.cache.hasOwnProperty("plugins/rr.js") || require.cache.hasOwnProperty("plugins/roulette.js") || require.cache.hasOwnProperty("plugins/safari.js") || require.cache.hasOwnProperty("plugins/mafia.js")) {
                commandsmessage += "<br>";
            }
        ...
        **/
        ...
        channels: function (src, channel, command) {
            ...
            for (i in channelList) {
                ...
                } else if (require.cache.hasOwnProperty("plugins/party.js") && ids[i] == partychannel) {
                    descriptions.push("To have some fun.");
                } else if (require.cache.hasOwnProperty("plugins/roulette.js") && ids[i] == roulettechannel) {
                    descriptions.push("To play Roulette.");
                } else if (require.cache.hasOwnProperty("plugins/rr.js") && ids[i] == rrchannel) {
                    descriptions.push("To play Russian Roulette.");
                } else if (require.cache.hasOwnProperty("plugins/safari.js") && ids[i] == safarichannel) {
                    descriptions.push("To play Safari.");
                } else if (require.cache.hasOwnProperty("plugins/mafia.js") && ids[i] == mafiachannel) {
        ...
        serverinfo: function (src, channel, command) {
            ...
            "<br><b>IP:</b> " + script.hostLocation.ip +
        ...
        scriptinfo: function (src, channel, command) {
            ...
            "<b>Enabled Plugins:</b> " + officialPlugins.join(", ") + "<br>" +
            (customPlugins.length > 0 ? "<b>Unofficial Plugins:</b> " + customPlugins.join(", ").replace(/\.js/g, "") + "<br>" : "") +
            "<h3>Contribution</h3>" +
        ...
        pokedex: function (src, channel, command) {
            ...
            if (helpers.isAndroidOrWeb(src)) {
                ...
                for (k in baseStats) {
                    stat = baseStats[k];
                    dexmessage += "<br><b>" + dex.statName(k) + "</b>" + helpers.spaces(8 - dex.statName(k).length) + " "
                    + dex.colorStat(stat)+ helpers.spaces(3 - stat.toString().length) + " "
                    + (k == '0' ? '-' + helpers.spaces(2) : dex.calcStat(k, stat, MAX_IV, MIN_EV, MIN_NATURE) + helpers.spaces(3 - dex.calcStat(k, stat, MAX_IV, MIN_EV, MIN_NATURE).toString().length)) + " | "
                    + dex.calcStat(k, stat, MAX_IV, MIN_EV, NEUTRAL_NATURE) + helpers.spaces(3 - dex.calcStat(k, stat, MAX_IV, MIN_EV, NEUTRAL_NATURE).toString().length) + " | "
                    + dex.calcStat(k, stat, MAX_IV, MAX_EV, NEUTRAL_NATURE) + helpers.spaces(3 - dex.calcStat(k, stat, MAX_IV, MAX_EV, NEUTRAL_NATURE).toString().length) + " | "
                    + (k == '0' ? '-' + helpers.spaces(2) : dex.calcStat(k, stat, MAX_IV, MAX_EV, MAX_NATURE));
                }
                ...
                for (k in baseStats) {
                    stat = baseStats[k];
                    dexmessage += "<tr><th>" + dex.statName(k) + "</th>"
                    + "<td>" + dex.colorStat(stat) + "</td>"
                    + "<td>" + (k == '0' ? '-' : dex.calcStat(k, stat, MAX_IV, MIN_EV, MIN_NATURE)) + "</td>"
                    + "<td>" + dex.calcStat(k, stat, MAX_IV, MIN_EV, NEUTRAL_NATURE) + "</td>"
                    + "<td>" + dex.calcStat(k, stat, MAX_IV, MAX_EV, NEUTRAL_NATURE) + "</td>"
                    + "<td>" + (k == '0' ? '-' : dex.calcStat(k, stat, MAX_IV, MAX_EV, MAX_NATURE)) + "</td></tr>";
                }
        ...
        movedex: function (src, channel, command) {
            ...
            power = dex.movePower(moveNum);
            category = dex.moveCategory(moveNum);
            accuracy = dex.moveAccuracy(moveNum);
            pp = dex.movePP(moveNum);
            effect = dex.moveEffect(moveNum);
            contact = dex.moveContact(moveNum);
            range = dex.moveRange(moveNum);
        ...
        abilitydex: function (src, channel, command) {
            ...
            + "<br><b>Description:</b> " + dex.ability(abilityNum)
            + "<br><b>Pokémon with this ability:</b> " + dex.pokemonWithAbility(abilityNum).join(", ")
            + "<br><br><timestamp/><br>" + border2;
        ...
        itemdex: function (src, channel, command) {
            ...
            itemmessage += "<br><b>Description:</b> " + (isBerry ? dex.getBerry(itemNum) : dex.getItem(itemNum))
            + "<br><b>Fling Power:</b> " + dex.getFlingPower(itemNum);
            if (isBerry) {
                itemmessage += "<br><b>Natural Gift Type:</b> " + typeImage(src, dex.getBerryType(itemNum))
                + "<br><b>Natural Gift Power:</b> " + dex.getBerryPower(itemNum);
            }
        ...
        **/
        ...
        selfmute: function (src, channel, command) {
            ...
            require("scripts/modcmds.js").commands.mute(src, channel, ["mute", sys.name(src), command[1]]);
            if (auth > 0) {
        ...
        selfban: function (src, channel, command) {
            ...
            require("scripts/admincmds.js").commands.ban(src, channel, ["ban", sys.name(src), command[1]]);
            if (auth > 0) {
